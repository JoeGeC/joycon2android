...
import com.joegec.joycon2android.dsu.LanAddress
...
fun DsuCard(
    ...
    FeatureToggleCard(
        ...
        subtitle = if (state.enabled) {
            stringResource(R.string.dsu_subtitle_on, DsuServer.PORT, state.clientCount)
        } else {
        ...
        AnimatedVisibility(
            ...
            Column {
                ...
                Spacer(Modifier.height(Dimens.elementSpacing))
                EmulatorConnection(state.lanEnabled)
                EmulatorGuides()
            }
        }
    }
...
@Composable
private fun EmulatorConnection(lanEnabled: Boolean) {
    val address = remember(lanEnabled) {
        val host = if (lanEnabled) LanAddress.firstIpv4() else "127.0.0.1"
        host?.let { "$it:${DsuServer.PORT}" }
    }
